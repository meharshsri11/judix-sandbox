...
export const Dropdown = ({
  ...
  if (isOpen && triggerRef.current) {
    ...
    // const positionStyles = getPositionStyles(triggerRect, position, align, offset); // Remove old call
    ...
    dropdownContent = createPortal(
      <div
        ref={dropdownRef}
        className={cn(
          "min-w-[150px] rounded-sm bg-neutral-100 shadow-default overflow-hidden",
          "animate-in fade-in-50 zoom-in-95",
          className
        )}
        style={{
          ...positionStyles, scrollbarColor: "var(--color-primary-400)",
          scrollbarWidth: "thin"
        }}
        role="menu"
  ...
  return (
    <div className="inline-block w-max text-sm">
      {triggerElement}
      {dropdownContent}
    </div>
  );
...
export const DropdownItem: React.FC<ComponentPropsWithoutRef<"button">> = ({
  className,
  children,
  ...props
}) => (
  <button
    className={cn(
      "flex w-full font-medium cursor-pointer items-center rounded-md px-4 py-3 text-body-sm leading-[130%] tracking-[4%]",
      "outline-none hover:bg-base-100 focus:bg-base-100",
      className
    )}
    role="menuitem"
    tabIndex={-1}
