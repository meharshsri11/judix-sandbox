  ...
  return (
    <section id="pricing" className="container mx-auto max-w-7xl">
      ...
      <div className="mb-12 flex items-center justify-center space-x-4">
        <span
          className={`font-outfit text-lg ${!isYearly ? 'text-white' : 'text-gray-400'}`}
        >
          Monthly
        </span>
        <Switch
        ...
        <span className="ml-2 rounded-full bg-green-500/20 px-3 py-1 text-sm font-semibold text-green-400">
          2 months FREE!
        </span>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        {pricingPlans.map((plan) => (
          ...
          >
            {plan.isPopular && (
              <div className="absolute -top-4 left-1/2 -translate-x-1/2 rounded-full bg-purple-500 px-4 py-1.5 font-sora text-sm font-semibold text-white">
                Most Popular
              </div>
            )}

            <h3 className="font-sora text-2xl font-semibold">{plan.name}</h3>
            <p className="mt-2 h-12 font-outfit text-gray-400">
              {plan.description}
            </p>

            <div className="mt-6">
              <span className="font-sora text-5xl font-bold">
